...
contract('YoloCoin', function(accounts) {
  ...
  it('initializes the contract with the correct values', function() {
    return YoloCoin.deployed().then(function(instance) {
      ...
      assert.equal(name, 'YOLO Coin', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'YOLO', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'YoloCoin-v1.0.0', 'has the correct standard');
    });
  })
